...
export default function Dashboard() {
  ...
  function exportCSV() {
    ...
    a.download = `cpp-leads-${new Date().toISOString().split('T')[0]}.csv`; a.click()
  }

  const statusColor = (s: string) => ({ pending: '#ffaa00', confirmed: ACCENT, completed: '#00d4ff', cancelled: '#ff4444', new: '#ffaa00' }[s] || '#666')
    ...
    <div style={{ minHeight: '100vh', background: '#080b0f', color: '#f0f4f8', fontFamily: 'monospace' }}>
      ...
      <div style={{ borderBottom: `2px solid ${ACCENT}`, padding: '16px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <div style={{ fontSize: '22px', fontWeight: 700, letterSpacing: '.05em' }}>CELL PHONE PARADISE</div>
        </div>
      ...
      {activeTab === 'leads' ? (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '1px', background: '#1a2332' }}>
            {[{ label: 'TODAY', value: stats.today }, { label: 'THIS WEEK', value: stats.week }, { label: 'ALL TIME', value: stats.total }].map(s => (
              <div key={s.label} style={{ background: '#080b0f', padding: '28px 32px' }}>
                ...
                <div style={{ fontSize: '10px', color: '#6b7a8d', marginTop: '4px' }}>BOOKINGS</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ margin: '24px 32px', border: '1px solid #1a2332', padding: '20px' }}>
            ...
            {overlayActive && (
              <div style={{ marginTop: '12px', padding: '10px 16px', background: 'rgba(57,255,20,.05)', border: `1px solid ${ACCENT}`, fontSize: '12px', color: ACCENT }}>
          ...
          <div style={{ margin: '0 32px 48px' }}>
            ...
            {loading ? (
              ...
              <div style={{ color: '#6b7a8d', padding: '48px', textAlign: 'center', border: '1px solid #1a2332' }}>NO BOOKINGS YET</div>
            ) : (
              <div style={{ border: '1px solid #1a2332' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr 1fr', padding: '10px 16px', background: '#0f1419', fontSize: '10px', color: '#6b7a8d', letterSpacing: '.1em', borderBottom: '1px solid #1a2332' }}>
                  <span>NAME</span><span>PHONE</span><span>DEVICE</span><span>SERVICE</span><span>STATUS</span>
                </div>
                {leads.map((lead, i) => (
                  <div key={lead.id} style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr 1fr', padding: '14px 16px', fontSize: '12px', borderBottom: i < leads.length - 1 ? '1px solid #0d1117' : 'none', background: i % 2 === 0 ? '#080b0f' : '#0a0e13' }}>
                    <span style={{ fontWeight: 600, color: '#f0f4f8' }}>{lead.name}</span>
                    <span style={{ color: '#6b7a8d' }}>{lead.phone || '—'}</span>
                    <span style={{ color: '#6b7a8d' }}>{lead.device || '—'}</span>
                    <span style={{ color: '#6b7a8d' }}>{lead.service || '—'}</span>
